...
def sheetRegister = ExcelFactory.getExcelDataWithDefaultSheet(
	GlobalVariable.FileProjectSimple + '\\ProjectSimple.xlsx', 'Register', true
)

for (def row = 1; row <= sheetRegister.getRowNumbers(); row++) {
	String firstname = sheetRegister.getValue(1, row) // kolom A
	String lastname = sheetRegister.getValue(2, row)  // kolom B
	String email = sheetRegister.getValue(3, row)     // kolom C
	String password = sheetRegister.getValue(4, row)  // kolom D

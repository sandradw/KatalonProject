...
def sheetLoginGagal= ExcelFactory.getExcelDataWithDefaultSheet(
	GlobalVariable.FileProjectSimple + '\\ProjectSimple.xlsx', 'LoginGagal', true
)

for (def row = 1; row <= sheetLoginGagal.getRowNumbers(); row++) {
	String email = sheetLoginGagal.getValue(1, row)     // kolom A
	String password = sheetLoginGagal.getValue(2, row)  // kolom B
